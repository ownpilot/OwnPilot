...
info "You need at least one AI provider API key to use OwnPilot."
echo

ask_secret "OpenAI API key (Enter to skip)" "" OPENAI_API_KEY
ask_secret "Anthropic API key (Enter to skip)" "" ANTHROPIC_API_KEY

if [[ -z "$OPENAI_API_KEY" && -z "$ANTHROPIC_API_KEY" ]]; then
  warn "No AI provider keys set. You'll need to add one to .env before using OwnPilot."
fi

# ─── Server Configuration ────────────────────────────────────────────────────
section "Server Configuration"

ask "HTTP port" "8080" PORT
ask "Bind address" "0.0.0.0" HOST
ask_choice "Environment" "development" NODE_ENV "development" "production"
ask "CORS origins (comma-separated)" "http://localhost:3000,http://localhost:5173" CORS_ORIGINS
...
echo -e "    ${CYAN}pnpm run dev${NC}       Start in development mode"
echo -e "    ${CYAN}pnpm run start${NC}     Start in production mode"
echo
echo -e "  ${BOLD}Useful commands:${NC}"
echo -e "    ${CYAN}pnpm run test${NC}      Run test suite"
echo -e "    ${CYAN}pnpm run lint${NC}      Check for lint errors"
echo
