...
- **Claw Runtime**: Unified autonomous agent composing LLM + workspace + soul + coding agents + 250+ tools. Types in `core/src/services/claw-types.ts`. Runner/Manager/Service in `gateway/src/services/claw-{runner,manager,service}.ts`. 6 tools in `tools/claw-tools.ts` (install_package, run_script, create_tool, spawn_subclaw, publish_artifact, request_escalation). DB: `claws`, `claw_sessions`, `claw_history` (migration 022). REST: `/api/v1/claws` (13 endpoints). UI: ClawsPage + ClawsWidget. 116 tests. Modes: `cyclic` (repeated) / `single-shot` (one execution). Subclaw depth limit: `MAX_CLAW_DEPTH=3`. Escalation: `escalation_pending` state + `POST /:id/approve-escalation`. Context: `claw-context.ts` (AsyncLocalStorage, like `heartbeat-context.ts`)
