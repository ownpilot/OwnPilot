/**
 * Coding Agents — Pipelines Tab
 *
 * Multi-step CLI tool orchestration. Set a goal, pick a provider
 * (Claude/Gemini/Codex), and let OwnPilot chain sessions together —
 * analyzing output and deciding next steps.
 *
 * Used by CodingAgentsPage as a tab; previously lived as a standalone
 * OrchestrationPage and got merged in to consolidate the
 * coding-agent surface area.
 */
...
export function PipelinesTab() {
    ...
    const toastEvents: Array<{ event: string; toast: (p: unknown) => void }> = [
      {
        ...
        toast: () => toast.info('Pipeline finished'),
      },
      {
        event: 'orchestration:cancelled',
        toast: () => toast.warning('Pipeline cancelled'),
      },
      {
        event: 'orchestration:error',
        toast: (p: unknown) => toast.error(`Pipeline error: ${(p as { error: string }).error}`),
      },
    ];
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Sub-toolbar */}
      <div className="flex items-center justify-between px-6 py-3 border-b border-border dark:border-dark-border bg-bg-secondary/40 dark:bg-dark-bg-secondary/40">
        <div className="text-xs text-text-muted dark:text-dark-text-muted">
          Multi-step CLI tool automation — chain Claude/Gemini/Codex sessions together
        </div>
            ...
            <div className="flex-1 flex items-center justify-center">
              <div className="text-center">
                ...
                <p className="text-sm text-text-muted dark:text-dark-text-muted">
                  Select a run or start a new pipeline
                </p>
