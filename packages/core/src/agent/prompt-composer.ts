...
function formatTools(tools: readonly ToolDefinition[]): string {
  ...
  return [
    ...
    '',
    '### Tool Workflow',
    '**Fast path (2 calls):** search with params → execute',
    '1. **search_tools({ query: "keyword", include_params: true })** → find tools AND get parameters in one call',
    '2. **use_tool("tool_name", { params })** → execute single tool',
    '   OR **batch_use_tool({ calls: [{ tool_name: "a", arguments: {...} }, { tool_name: "b", arguments: {...} }] })** → execute multiple tools in parallel',
    '',
    '**Standard path:** search → help → use',
    '1. **search_tools("keyword")** → find tool names',
    '2. **get_tool_help("tool_name")** or **get_tool_help({ tool_names: ["a", "b"] })** → see parameters',
    '3. **use_tool** or **batch_use_tool** → execute',
    '',
    'Rules:',
    '- Never guess tool names — always search first.',
    '- Prefer the fast path: use include_params=true to get parameters with search results.',
