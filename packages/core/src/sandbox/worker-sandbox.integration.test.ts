...
/**
 * Per-test budget for the real-worker cases.
 *
 * These spawn an actual Node worker thread, so their wall time depends on
 * machine contention rather than on the assertion. `pnpm test` runs all four
 * packages concurrently, and under that load worker spawn plus module load
 * exceeded the previous 20s — this file failed only in the full run and passed
 * every time core was run alone. A timeout that a test only misses when the
 * machine is busy is measuring the machine, so give it real headroom.
 */
const WORKER_TEST_TIMEOUT = 45_000;

let cachedRunner: RunInWorkerSandbox | null = null;
...
async function loadRunner(): Promise<RunInWorkerSandbox> {
  ...
  cachedRunner = mod.runInWorkerSandbox;
  return cachedRunner;
}

describe.skipIf(!DIST_BUILT)('WorkerSandbox real-worker integration (build-gated)', () => {
  // Pay the dist import once in a hook rather than inside whichever test runs
  // first, so no single test is charged for it.
  beforeAll(async () => {
    await loadRunner();
  }, WORKER_TEST_TIMEOUT);

  it(
    'runs code in a real worker thread and returns the value',
    async () => {
  ...
  it(
    'rebuilds standard globals (crypto) natively inside the worker',
    async () => {
      const runInWorkerSandbox = await loadRunner();
  ...
  it(
    'ENFORCES maxMemory: a runaway allocation kills the worker, host survives',
    async () => {
      const runInWorkerSandbox = await loadRunner();
  ...
  it(
    'bridges a host-state function over RPC (config-style read round-trips)',
    async () => {
      const runInWorkerSandbox = await loadRunner();
      const result = await runInWorkerSandbox(
        'test:integration' as PluginId,
  ...
  it(
    'tool-profile crypto.createHash survives the membrane (de-risks custom-tool wiring)',
    async () => {
      const runInWorkerSandbox = await loadRunner();
  ...
  it(
    'tool-profile crypto convenience helpers (sha256/randomUUID) work in-worker',
    async () => {
      const runInWorkerSandbox = await loadRunner();
      const result = await runInWorkerSandbox(
        'test:integration' as PluginId,
  ...
  it(
    'runs a full custom-tool-shaped payload in-worker (utils + crypto + __args__ + RPC)',
    async () => {
      const runInWorkerSandbox = await loadRunner();
      const code = `
  ...
  it(
    'executeDynamicTool routes a real custom tool through the worker when opted in',
    async () => {
  ...
  it(
    'propagates a host-handler error back into the sandbox',
    async () => {
      const runInWorkerSandbox = await loadRunner();
      const result = await runInWorkerSandbox(
        'test:integration' as PluginId,
