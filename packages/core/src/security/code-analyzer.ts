...
const RISK_PATTERNS: readonly RiskPattern[] = [
  ...
  {
    regex: /\bimport\s+.*child_process/,
    description: 'Child process import',
    severity: 'high',
    score: 30,
    languages: ['javascript'],
  },
  ...
  {
    regex: /\bfs\.(writeFile|appendFile|unlink|rmdir|rm)\b/,
  ...
  {
    regex: /\bshutil\.(rmtree|move|copy)\b/,
  ...
  {
    regex: /\bctypes\b/,
    description: 'C-level API access',
    severity: 'high',
    score: 25,
    languages: ['python'],
  },
  ...
  {
    regex: /\brequests\.(get|post|put|delete|patch)\b/,
  ...
  {
    regex: /\bimport\s+.*\bfs\b/,
    description: 'Filesystem import',
    severity: 'medium',
    score: 10,
    languages: ['javascript'],
  },
  ...
  {
    regex: /\bconsole\.(log|warn|error|info)\b/,
  ...
  if (critical.blocked) {
    return {
      ...
      factors: [
        {
          pattern: 'critical',
          description: critical.reason ?? 'Critical pattern detected',
          severity: 'critical',
        },
      ],
      blocked: true,
