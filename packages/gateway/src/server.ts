...
async function main() {
  ...
  if (config.auth?.type === 'none' || !config.auth?.type) {
    ...
    // Passwordless localhost installs retain the convenient implicit-owner
    // flow. On exposed binds, app.ts now defers to the configured API auth and
    // WebSocket auth validates the same resolved API-key/JWT configuration.
    // Warn because the browser UI still needs a UI password for cookie-based
    // login instead of a programmatic Authorization header.
    const { isPasswordConfigured } = await import('./services/ui-session.js');
    if (!isPasswordConfigured()) {
      const isExposed = host !== '127.0.0.1' && host !== 'localhost' && host !== '::1';
      if (isExposed) {
        log.warn(
          `No UI password set on exposed HOST=${host}. AUTH_TYPE=${config.auth.type} still protects HTTP and WebSocket APIs, but browser UI login requires a UI password.`
        );
      } else {
        log.warn(
