...
describe("Health Routes", () => {
  ...
  describe("GET /health", () => {
    ...
    it("reports healthy when all checks pass", async () => {
      ...
      mockGetSandboxStatus.mockResolvedValue({ dockerAvailable: true, dockerVersion: "24.0.5", relaxedSecurityRequired: false });
      const res = await app.request("/health");
      const json = await res.json();
    ...
    it("reports degraded when docker not available but mode is not docker", async () => {
      mockIsConnected.mockReturnValue(true);
      mockGetExecutionMode.mockReturnValue("local");
      mockGetSandboxStatus.mockResolvedValue({ dockerAvailable: false, dockerVersion: null, relaxedSecurityRequired: false });
      const res = await app.request("/health");
      const json = await res.json();
    ...
    it("handles getAdapterSync throwing", async () => {
      mockGetAdapterSync.mockImplementation(() => { throw new Error("Not init"); });
      const res = await app.request("/health");
      const json = await res.json();
      expect(res.status).toBe(200);
    ...
    it("shows strict security mode when docker available and not relaxed", async () => {
      mockIsConnected.mockReturnValue(true);
      mockGetSandboxStatus.mockResolvedValue({ dockerAvailable: true, dockerVersion: "25.0.0", relaxedSecurityRequired: false });
      const res = await app.request("/health");
      const json = await res.json();
    ...
    it("shows relaxed security mode", async () => {
      mockIsConnected.mockReturnValue(true);
      mockGetSandboxStatus.mockResolvedValue({ dockerAvailable: true, dockerVersion: "24.0.0", relaxedSecurityRequired: true });
      const res = await app.request("/health");
      const json = await res.json();
    ...
    it("shows database host in message when host is present", async () => {
      mockIsConnected.mockReturnValue(true);
      mockGetDatabaseConfig.mockReturnValue({ postgresHost: "db.example.com", postgresPort: 5432, postgresUser: "user", postgresDatabase: "mydb" });
      const res = await app.request("/health");
      const json = await res.json();
      const dbCheck = json.data.checks.find((c: { name: string }) => c.name === "database");
    ...
    it("shows database message without host when host is empty", async () => {
      mockIsConnected.mockReturnValue(true);
      mockGetDatabaseConfig.mockReturnValue({ postgresHost: "", postgresPort: 5432, postgresUser: "user", postgresDatabase: "mydb" });
      const res = await app.request("/health");
      const json = await res.json();
      const dbCheck = json.data.checks.find((c: { name: string }) => c.name === "database");
    ...
    it("shows docker version as unknown when dockerVersion is null", async () => {
      mockGetSandboxStatus.mockResolvedValue({ dockerAvailable: true, dockerVersion: null, relaxedSecurityRequired: false });
      const res = await app.request("/health");
      const json = await res.json();
      const dockerCheck = json.data.checks.find((c: { name: string }) => c.name === "docker");
  ...
  describe("GET /health/sandbox", () => {
    it("returns sandbox status without refresh", async () => {
      const sandboxData = { dockerAvailable: true, dockerVersion: "24.0.5", relaxedSecurityRequired: false };
  ...
  describe("POST /health/sandbox/pull-images", () => {
    ...
    it("reports failure when ensureImage throws", async () => {
      mockEnsureImage.mockResolvedValueOnce(true).mockRejectedValueOnce(new Error("Pull failed")).mockResolvedValueOnce(false);
      const res = await app.request("/health/sandbox/pull-images", { method: "POST" });
      expect(res.status).toBe(200);
      const json = await res.json();
      expect(json.data.images.python).toEqual({ success: true });
