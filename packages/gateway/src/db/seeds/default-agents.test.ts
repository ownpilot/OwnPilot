...
describe('getDefaultAgents', () => {
  ...
  it('caches result on second call', async () => {
    ...
    mockReadFileSync.mockReturnValue(JSON.stringify({
      version: '1.0',
      agents: [{ id: 'a1', name: 'A', category: 'c', systemPrompt: 's', config: { maxTokens: 1000, temperature: 0.5, maxTurns: 5, maxToolCalls: 10 } }],
    }));
    const { getDefaultAgents } = await import('./default-agents.js');
    getDefaultAgents();
    getDefaultAgents();
...
describe('DEFAULT_AGENTS proxy', () => {
  ...
  it('exposes agents via array index access', async () => {
    ...
    mockReadFileSync.mockReturnValue(JSON.stringify({
      version: '1.0',
      agents: [
        { id: 'proxy-1', name: 'Proxy Agent', category: 'test', systemPrompt: 'Test', config: { maxTokens: 1024, temperature: 0.7, maxTurns: 5, maxToolCalls: 10 } },
      ],
    }));
  ...
  it('exposes length property', async () => {
    ...
    mockReadFileSync.mockReturnValue(JSON.stringify({
      version: '1.0',
      agents: [
        { id: 'a', name: 'A', category: 'c', systemPrompt: 's', config: { maxTokens: 1024, temperature: 0.5, maxTurns: 3, maxToolCalls: 5 } },
        { id: 'b', name: 'B', category: 'c', systemPrompt: 's', config: { maxTokens: 1024, temperature: 0.5, maxTurns: 3, maxToolCalls: 5 } },
      ],
    }));
    const { DEFAULT_AGENTS } = await import('./default-agents.js');
  ...
  it('is iterable', async () => {
    ...
    mockReadFileSync.mockReturnValue(JSON.stringify({
      version: '1.0',
      agents: [
        { id: 'iter-1', name: 'Iter Agent', category: 'test', systemPrompt: 'T', config: { maxTokens: 512, temperature: 0.5, maxTurns: 2, maxToolCalls: 5 } },
      ],
    }));
    const { DEFAULT_AGENTS } = await import('./default-agents.js');
